...
public class MeetingsConfiguration {
    ...
    public MeetingsFacade inMemoryMeetingsFacade(ActiveSubscribersFinder activeSubscribersFinder, Calendar calendar) {
        var meetingsScheduler = new MeetingsScheduler(activeSubscribersFinder, meetingRepository, meetingGroupRepository, calendar);
        return new LogsDecorator(new MeetingsFacadeImpl(meetingGroupRepository, meetingsScheduler));
    }
}